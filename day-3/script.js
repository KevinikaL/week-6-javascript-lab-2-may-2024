...
function partThreeB() {
  // 1. prompt for a friend
  let friend = prompt("What's your best friend's name?");
  // 2. add the friend to the message
  messageParagraph.innerHTML += `, ${friend}`;
}
...
function partFourA() {
  ...
  // 1. prompt for a friend
  newBestFriend = prompt(
    `You have ${totalBestFriends} best friends. To add another, enter a friend's name.`
  );
...
function partFourB() {
  ...
  newFavFlavor = prompt(
    `You have ${totalFavFlavors} favorite flavors. Enter another favorite flavor.`
  );
